...
namespace TestApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var apiKey = "[SearchServiceApiKey]";

            var dataSourceName = "[DataSourceName]";
            var indexName = "[IndexName]";
            var indexerName = "[IndexerName]";

            var blobStorageConnectionString = "[BlobStorageConnectionString]";
            var blobStorageContainerName = "[BlobStorageContainerName]";

            using (var httpClient = new HttpClient())
            {
                var dataSourceDefinition = AzureBlobDatasourceDefinition(blobStorageConnectionString, blobStorageContainerName);
        ...
        static string AzureBlobDatasourceDefinition(string connectionString, string containerName)
        {
            return @"
                    {
        ...
        static string IndexerDefinition(string dataSourceName, string indexName)
        {
            return @"
                    {
                      ""description"": ""indexer for azure blob datasource"",
